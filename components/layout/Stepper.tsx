...
const Stepper: React.FC<StepperProps> = ({ steps, currentPath, headerHeight }) => {
          ...
          return (
            <li key={step.shortTitle} className="flex-shrink-0">
              <button
                ...
                disabled={isStart}
                className={`group flex flex-col items-center py-2 px-1 md:px-3 border-b-4 focus:outline-none ${isCurrent ? 'border-sky-500 text-sky-600' : isCompleted ? 'border-green-500 text-green-600' : 'border-transparent text-slate-500'} ${isStart ? 'cursor-default' : 'hover:border-sky-300 hover:text-slate-700'}`}
              >
