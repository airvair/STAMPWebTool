// airvair/stampwebtool/STAMPWebTool-a2dc94729271b2838099dd63a9093c4d/components/step3_ControlStructure/partials/FeedbackPathsBuilder.tsx
        ...
        <section className="space-y-6">
            ...
            <div className="bg-slate-100 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700/50 space-y-4">
                ...
                <div className="flex space-x-2 pt-4">
                    <Button onClick={handleSaveFeedbackPath} leftIcon={<PlusIcon className="w-5 h-5" />}>
                        {editingFpId ? 'Update Path' : 'Add Path'}
                    </Button>
                    {editingFpId && <Button onClick={resetForm} variant="secondary">Cancel</Button>}
                </div>
            </div>
                ...
                {feedbackPaths.map(fp => (
                    <li key={fp.id} className={`flex justify-between items-center p-3 border rounded-md bg-white dark:bg-slate-900 shadow-sm ${fp.isMissing ? 'border-dashed border-red-500/50' : 'border-slate-200 dark:border-slate-700'}`}>
                        ...
                        <div className="flex items-center space-x-1 ml-4">
                            ...
                            <Button onClick={() => deleteFeedbackPath(fp.id)} size="sm" variant="ghost" className="text-red-500"><TrashIcon className="w-4 h-4" /></Button>
                        </div>
                    </li>
                ))}
            </ul>
