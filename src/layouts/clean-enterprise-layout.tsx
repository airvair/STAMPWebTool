...
import { MalmquistLogo } from '@/components';
...
const ANALYSIS_STEPS: { step: AnalysisStep; title: string; shortTitle: string }[] = [
  { step: 'scope', title: 'Scope, Losses, Hazards & Constraints', shortTitle: 'Scope & Losses' },
  {
    ...
    title: 'Control Structure & Actions',
    shortTitle: 'Structure & Actions',
  },
...
const CleanEnterpriseLayout: React.FC = () => {
  ...
  const { currentProject, deleteAnalysis, selectAnalysis, updateAnalysis, reorderAnalyses } =
    useProjects();
  ...
  const [analysisToDelete, setAnalysisToDelete] = useState<{ id: string; title: string } | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      handleWorkspaceSectionChange as EventListener
    );
    ...
    <FeedbackContainer>
      <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-white to-neutral-100 dark:from-black dark:via-neutral-950 dark:to-neutral-900">
        ...
        <div className="pointer-events-none fixed inset-0 overflow-hidden">
          <div className="absolute -top-1/2 -left-1/2 h-full w-full rounded-full bg-gradient-to-br from-sky-100/10 to-transparent blur-3xl dark:from-sky-900/5" />
          <div className="absolute -right-1/2 -bottom-1/2 h-full w-full rounded-full bg-gradient-to-tl from-purple-100/10 to-transparent blur-3xl dark:from-purple-900/5" />
        </div>
                                  ...
                                  {renamingAnalysisId === analysis.id ? (
                                    ...
                                    >
                                      <input
                                        ...
                                        className="border-sidebar-border focus:border-sidebar-accent flex-1 border-b bg-transparent text-sm focus:outline-none"
                                        autoFocus
                                      />
                                    </form>
                                  ) : (
                                    <>
                                      <SidebarMenuButton
                                        ...
                                        isActive={isSelected}
                                        className="group flex h-auto min-h-[2rem] items-center py-2"
                                      >
                                        <div className="flex min-w-0 flex-1 items-center gap-2">
                                          <div
                                            ...
                                            className="chevron-button hover:bg-sidebar-accent cursor-pointer rounded p-0.5 transition-colors"
                                          ...
                                          <div className="flex-1">
                                            <span className="line-clamp-2 block py-0.5">
                                              {analysis.title}
                                            </span>
                                          </div>
                                        </div>
                                        <span
                                          className={`ml-2 flex-shrink-0 text-xs font-bold ${
                                            analysis.analysisType === AnalysisType.CAST
                                              ? 'text-pink-500'
                                              : 'text-blue-400'
                                          }`}
                                        >
                                      ...
                                      <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                          <SidebarMenuAction className="dropdown-trigger !top-1/2 !-translate-y-1/2">
                                        ...
                                        <DropdownMenuContent align="start" side="right">
                                          ...
                                          <DropdownMenuItem
                                            onClick={() =>
                                              handleDeleteAnalysisFromMenu(analysis.id)
                                            }
                                      ...
                                      return (
                                        <AnimatedCollapsibleItem key={step.step}>
                                          <SidebarMenuSubItem data-step-index={index}>
                                            ...
                                            >
                                              <Icon
                                                className={`h-3 w-3 shrink-0 ${isCompleted ? 'text-green-600 dark:text-green-400' : ''}`}
                                              />
                                          ...
                                          <AnimatedCollapsible
                                            isOpen={isScopeStep && isCurrent && isSelected}
                                            className="ml-2"
                                          >
                                            <SidebarMenuSub
                                              showProgress={true}
                                                    ...
                                                    <SidebarMenuSubButton
                                                      onClick={() => {
                                                        ...
                                                        window.dispatchEvent(
                                                          new CustomEvent('cast-substep-change', {
                                          ...
                                          <AnimatedCollapsible
                                            isOpen={isStructureStep && isCurrent && isSelected}
                                            className="ml-2"
                                          >
                                            <SidebarMenuSub
                                              showProgress={true}
                                              totalSteps={6}
                                              completedSteps={
                                                activeWorkspaceSection === 'components'
                                                  ? 1
                                                  : activeWorkspaceSection === 'controllers'
                                                    ? 2
                                                    : activeWorkspaceSection === 'control-paths'
                                                      ? 3
                                                      : activeWorkspaceSection === 'feedback-paths'
                                                        ? 4
                                                        : activeWorkspaceSection === 'failure-paths'
                                                          ? 5
                                          ...
                                          <AnimatedCollapsible
                                            isOpen={isUCAStep && isCurrent && isSelected}
                                            className="ml-2"
                                          >
                                            <SidebarMenuSub
                                              showProgress={true}
                                              totalSteps={2}
                                              completedSteps={
                                                activeUCASection === 'ucas'
                                                  ? 1
                                                  : activeUCASection === 'uccas'
                                                    ? 2
                                                    : 0
                                              }
                                              currentStep={
                                                  ...
                                                  <SidebarMenuSubItem>
                                                    <SidebarMenuSubButton
                                                      onClick={() => {
                                                        ...
                                                        window.dispatchEvent(
                                                          new CustomEvent('uca-section-change', {
                                                            detail: { section: section.id },
                                                          })
                                                        );
                                                      }}
                  ...
                  <SidebarMenuItem>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <SidebarMenuButton
                          size="lg"
                          className="hover:bg-sidebar-accent hover:text-sidebar-accent-foreground w-full justify-between"
                        >
        ...
        <ConfirmationDialog
          ...
          description={
            analysisToDelete
              ? `You are about to permanently delete the analysis "${analysisToDelete.title}". This action cannot be undone and all associated data will be permanently removed from the system.`
              : ''
          }
