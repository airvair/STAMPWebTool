import { ValidationResult, ValidationError, ValidationWarning } from './uca-validation';
...
export class ValidationErrorCollection extends Error {
  ...
  constructor(validationResult: ValidationResult, context: ErrorContext) {
    ...
    super(`Validation failed with ${errorCount} error(s) and ${warningCount} warning(s)`);
  ...
  private formatWarningsForUser(warnings: ValidationWarning[]): string {
    ...
    return `Please review the following:\n${warnings
      .map((w, i) => `${i + 1}. ${w.message}${w.recommendation ? ` (${w.recommendation})` : ''}`)
      .join('\n')}`;
  }
...
export class ErrorHandler {
  ...
  public static handleValidationResult(result: ValidationResult, context: ErrorContext): boolean {
    if (!result.valid) {
    ...
    if (result.warnings.length > 0) {
      const feedback: UserFeedback = {
        ...
        message: result.warnings
          .map(w => (w.recommendation ? `${w.message} (${w.recommendation})` : w.message))
          .join('\n'),
        autoClose: true,
        duration: 8000,
      };
