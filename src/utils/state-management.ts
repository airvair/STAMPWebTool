...
import { ErrorHandler, SafetyAnalysisError, createErrorContext } from './error-handling';
import { ValidationResult } from './uca-validation';
...
export class AnalysisStateManager {
  ...
  async updateUCA(
    ...
    try {
      ...
      const validation = await this.validateUCAData(
        updatedUCA,
        controllers,
        controlActions,
        hazards
  ...
  private async validateUCAData(
    ...
    const { validateUCA } = await import('./uca-validation');
  ...
  private generateUCACode(
    ucaData: Omit<UnsafeControlAction, 'id' | 'code'>,
    controlActions: ControlAction[]
  ): string {
    const action = controlActions.find(ca => ca.id === ucaData.controlActionId);
    const actionCode = action
      ? `${action.verb.substring(0, 3).toUpperCase()}${action.object.substring(0, 3).toUpperCase()}`
      : 'UCA';
  ...
  private handleEntityError(error: Error, message: string, operationId: string): void {
    ...
    ErrorHandler.handleError(
      error,
      createErrorContext('AnalysisStateManager', 'handleEntityError')
    );
  }
  ...
  private findTraceabilityNode(entityId: string): TraceabilityNode | null {
    ...
    const types: TraceabilityLink['sourceType'][] = [
      'loss',
      'hazard',
      'uca',
      'scenario',
  ...
  private getEntityAnalysisStatus(
    ...
    const hasDownstream = this.state.traceabilityLinks.some(link => link.sourceId === entity.id);
    const hasUpstream = this.state.traceabilityLinks.some(link => link.targetId === entity.id);
