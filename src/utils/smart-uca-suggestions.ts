...
// Minimal implementation to replace removed mit-stpa-compliance
...
const performSystematicCompletenessCheck = (
  controllers: Controller[],
  controlActions: ControlAction[],
  ucas: UnsafeControlAction[],
  hazards: Hazard[]
): CompletenessCheckResult => {
  const missingCombinations: CompletenessCheckResult['missingCombinations'] = [];
  const qualityIssues: CompletenessCheckResult['qualityIssues'] = [];

  // Check for missing UCA combinations
  ...
  const existingUCAs = new Set(
    ucas.map(u => `${u.controllerId}-${u.controlActionId}-${u.ucaType}`)
  );

  for (const controller of controllers) {
    for (const action of controlActions) {
      if (action.controllerId === controller.id) {
        for (const ucaType of ucaTypes) {
          const key = `${controller.id}-${action.id}-${ucaType}`;
          if (!existingUCAs.has(key)) {
  ...
  // Check quality of existing UCAs
  for (const uca of ucas) {
    if (!uca.context || uca.context.length < 10) {
      qualityIssues.push({
        ucaId: uca.id,
        issue: 'Context description is too brief',
        severity: 'high',
      });
    }
    if (!uca.hazardIds || uca.hazardIds.length === 0) {
      qualityIssues.push({
        ucaId: uca.id,
        issue: 'No hazards linked to this UCA',
        severity: 'high',
      });
    }
  }

  return { missingCombinations, qualityIssues };
};
...
export const generateSmartUcaSuggestions = (
  ...
  const completenessCheck = performSystematicCompletenessCheck(
    controllers,
    controlActions,
    ucas || [],
    hazards
