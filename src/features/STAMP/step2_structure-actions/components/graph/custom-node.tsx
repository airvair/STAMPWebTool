...
import { NODE_WIDTH, CHILD_NODE_SPACING } from '@/utils/constants';
...
export const CustomNode: React.FC<NodeProps<CustomNodeData>> = ({ data }) => {
  ...
  const renderDynamicHandles = () => {
    ...
    if (hasChildren && data.children!.length <= 1 && !hasGrandchildren && !hasMultipleParents)
      return null;
    ...
    if (hasChildren) {
      data.children!.forEach((childId, index) => {
        ...
        const childCenterX =
          grandchildWidth + index * (childWidth + CHILD_NODE_SPACING) + childWidth / 2;
    ...
    if (hasMultipleParents) {
      data.parents!.forEach((parentId, index) => {
        ...
        for (let i = 0; i < index; i++) {
          ...
          const parentWidth =
            data.parentWidths && data.parentWidths[i] ? data.parentWidths[i] : NODE_WIDTH;
        ...
        const currentParentWidth =
          data.parentWidths && data.parentWidths[index] ? data.parentWidths[index] : NODE_WIDTH;
