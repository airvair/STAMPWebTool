...
import { SystemComponent, ComponentType } from '@/types/types';
...
const SystemComponentsBuilder: React.FC = () => {
  ...
  useEffect(() => {
    if (systemComponents.length === 0 && hazards.length > 0) {
      const componentNamesFromHazards = new Set(
        hazards.map(h => h.systemComponent.trim()).filter(Boolean)
      );
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          Start at the bottom of the ladder. What are the basic physical parts or processes your
          system controls? (e.g., &apos;The Aircraft&apos;, &apos;The Patient&apos;, &apos;The
          Database&apos;)
        </p>
      </div>
      ...
      <div className="w-full max-w-3xl rounded-lg border border-slate-200 bg-slate-100 p-4 dark:border-slate-700/50 dark:bg-slate-800/50">
        <div className="flex flex-wrap items-end gap-4 md:flex-nowrap">
          <Input
            ...
            placeholder="e.g., Engine, Database"
            containerClassName="w-full md:w-auto flex-grow !mb-0"
          />
          <Select
          ...
          {editingComponentId && (
            <Button onClick={resetForm} variant="secondary" className="h-10">
              Cancel
            </Button>
          )}
        </div>
