...
import { CONTROLLER_TYPE_COLORS } from '@/utils/constants';
...
const TeamStructureEditor: React.FC<{
  ...
  const deleteContext = (id: string) => {
    onTeamDetailsChange({
      ...teamDetails,
      contexts: teamDetails.contexts.filter(c => c.id !== id),
    });
  };
  ...
  return (
    <div className="mt-4 space-y-6 border-t border-slate-300 pt-4 dark:border-neutral-700">
      {/* 1. Member Definition */}
      <div>
        <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">
          1. Define Team Members
        </label>
        <div className="rounded-lg border border-slate-200 bg-slate-100 p-4 dark:border-slate-700/50 dark:bg-slate-800/50">
          <div className="space-y-2">
            <div className="flex items-center">
              <input
                type="radio"
                name="team-hierarchy"
                checked={!teamDetails.isHierarchical}
                onChange={() => handleHierarchyChange(false)}
                className="h-4 w-4 border-neutral-600 text-sky-600 focus:ring-sky-500"
              />
              <label className="ml-2 text-sm text-slate-700 dark:text-slate-300">
                Team members are peers (equal rank)
              </label>
            </div>
            <div className="flex items-center">
              <input
                ...
                name="team-hierarchy"
                checked={!!teamDetails.isHierarchical}
                onChange={() => handleHierarchyChange(true)}
                className="h-4 w-4 border-neutral-600 text-sky-600 focus:ring-sky-500"
              />
              <label className="ml-2 text-sm text-slate-700 dark:text-slate-300">
                Team has a formal hierarchy (different ranks)
              </label>
            </div>
          </div>
          <div className="mt-4">
            {teamDetails.isHierarchical ? (
              <div className="space-y-2">
                <ul className="space-y-1">
                  {teamDetails.members.map(m => (
                    <li
                      key={m.id}
                      className="flex items-center justify-between rounded border border-slate-300 bg-white p-2 text-sm dark:border-neutral-700 dark:bg-neutral-900"
                    >
                      <span>
                        {m.name} ({m.commandRank})
                      </span>
                      <Button
                ...
                <div className="flex items-end space-x-2 pt-2">
                  ...
                  <Select
                    ...
                    options={commandRankOptions}
                    containerClassName="flex-grow !mb-0"
                  />
                  <Button onClick={addHierarchicalMember} className="h-10">
                    Add
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-end space-x-2">
                  ...
                  />
                  <Button onClick={generatePeerMembers} className="h-10">
                    Generate
                  </Button>
                </div>
                <ul className="space-y-1 pt-2">
                  {teamDetails.members.map(m => (
                    <li
                      key={m.id}
                      className="flex items-center justify-between rounded border border-slate-300 bg-white p-1 dark:border-neutral-700 dark:bg-neutral-900"
                    >
                      <Input
                        value={m.name}
      ...
      <div>
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-100 p-4 dark:border-slate-700/50 dark:bg-slate-800/50">
          <p className="mb-2 text-xs text-slate-600 dark:text-slate-400">
            Define operational roles (e.g., &quot;Pilot Flying&quot;, &quot;Surgeon on Call&quot;)
            and their authority level for the diagram (lower number is higher).
          </p>
          <ul className="my-2 space-y-1">
            {teamDetails.roles.map(r => (
              ...
              >
                <span>
                  {r.name} (Auth: {r.authorityLevel ?? 'N/A'})
                </span>
                <Button
          ...
          <div className="mt-3 flex items-end space-x-2">
            <Input
              ...
              placeholder="e.g., Pilot Flying"
              containerClassName="!mb-0 flex-grow"
            />
            <Input
              label="Authority Level"
              type="number"
              value={newRoleAuthLevel}
              onChange={e => setNewRoleAuthLevel(parseInt(e.target.value))}
              placeholder="e.g., 10"
      ...
      <div>
        <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">
          3. Define Operational Contexts & Assign Roles
        </label>
        <div className="rounded-lg border border-slate-200 bg-slate-100 p-4 dark:border-slate-700/50 dark:bg-slate-800/50">
          <p className="mb-2 text-xs text-slate-600 dark:text-slate-400">
            Define specific situations (e.g., &quot;Takeoff Phase&quot;, &quot;Emergency
            Shutdown&quot;) and assign a role to each member within that context.
          </p>
          <ul className="my-2 space-y-2">
            {teamDetails.contexts.map(ctx => (
              <li
                key={ctx.id}
                className="rounded border border-slate-300 bg-white p-3 dark:border-neutral-700 dark:bg-neutral-900"
              >
                <div className="mb-2 flex items-center justify-between">
                  <p className="font-medium text-slate-800 dark:text-slate-100">{ctx.name}</p>
                  <Button
                  ...
                  {teamDetails.members.map(member => (
                    <Select
                      ...
                      options={[
                        ...
                        ...teamDetails.roles.map(r => ({ value: r.id, label: r.name })),
                      ]}
                      containerClassName="!mb-0"
                    />
                  ))}
          ...
          <div className="mt-3 flex items-end space-x-2">
            <Input
              ...
              placeholder="e.g., Takeoff Phase"
              containerClassName="!mb-0 flex-grow"
            />
            <Button onClick={addContext} className="h-10">
              Add Context
            </Button>
          </div>
        </div>
...
const ControllersBuilder: React.FC = () => {
  ...
  const handleSaveController = () => {
    ...
    const controllerData: Omit<Controller, 'id'> = {
      name: controllerName,
  ...
  const controllerTypeOptions = Object.values(ControllerType).map(ct => ({
    value: ct,
    label: `${ct} - ${ct === 'S' ? 'Software' : ct === 'H' ? 'Human' : ct === 'T' ? 'Team' : 'Organization'}`,
  }));
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          Now, who or what pulls the levers? Add the controllers that manage your components. This
          can be a person, software, a team, or even an entire organization.
        </p>
      </div>
      <div className="w-full max-w-3xl rounded-lg border border-slate-200 bg-slate-100 p-4 dark:border-slate-700/50 dark:bg-slate-800/50">
        <div className="grid grid-cols-1 items-end gap-4 md:grid-cols-2">
          <Input
            label="Controller Name"
            value={controllerName}
            onChange={e => setControllerName(e.target.value)}
            placeholder="e.g., Pilot, ECU, Safety Board"
          />
          <Select
        ...
        {controllerType === ControllerType.Team && (
          <div className="mt-4 border-t border-slate-300 pt-4 dark:border-neutral-700">
            <label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200">
              How would you like to model this team?
            </label>
            <div className="space-y-2">
              <div className="flex items-center">
                <input
                  ...
                  onChange={() => setTeamDetails(d => ({ ...d, isSingleUnit: true }))}
                  className="h-4 w-4 border-neutral-600 text-sky-600 focus:ring-sky-500"
                />
                <label className="ml-2 text-sm text-slate-700 dark:text-slate-300">
              ...
              <div className="flex items-center">
                <input
                  ...
                  name="team-model"
                  checked={!teamDetails.isSingleUnit}
                  onChange={() => setTeamDetails(d => ({ ...d, isSingleUnit: false }))}
                  className="h-4 w-4 border-neutral-600 text-sky-600 focus:ring-sky-500"
                />
                <label className="ml-2 text-sm text-slate-700 dark:text-slate-300">
                  As a group of individual members with roles/contexts
                </label>
              </div>
            </div>
        ...
        <div className="mt-4 flex space-x-2 border-t border-slate-300 pt-4 dark:border-neutral-700">
          <Button onClick={handleSaveController} leftIcon={<PlusIcon className="h-5 w-5" />}>
      ...
      <ul className="space-y-2">
        {controllers.map(ctrl => (
          <li
            key={ctrl.id}
            className={`rounded-md border p-3 shadow-sm ${CONTROLLER_TYPE_COLORS[ctrl.ctrlType]}`}
          >
            <div className="flex w-full items-start justify-between">
              <div>
                <p className="font-medium">
                  {ctrl.name}{' '}
                  <span className="font-mono text-xs opacity-80">({ctrl.ctrlType})</span>
                </p>
                {ctrl.ctrlType === ControllerType.Team && !ctrl.teamDetails?.isSingleUnit && (
                  <p className="mt-1 text-xs opacity-90">
                    {ctrl.teamDetails?.members.length || 0} members defined
                  </p>
                )}
              </div>
              <div className="ml-4 flex flex-shrink-0 items-center space-x-1">
                <Button onClick={() => editController(ctrl)} size="sm" variant="ghost">
                  Edit
                </Button>
                <Button
