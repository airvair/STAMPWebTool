        ...
        if (parent) {
          ...
          if (siblings.length > 1 && siblingIndex !== -1) {
            ...
            const totalChildrenWidth =
              siblings.length * NODE_WIDTH + (siblings.length - 1) * CHILD_NODE_SPACING;
          ...
          if (nodeA.position.x < nodeB.position.x) {
            ...
            commonChildren.forEach(childId => {
              ...
              if (child) {
                ...
                const childWidth =
                  child.data?.width || (child.style?.width as number) || NODE_WIDTH;
                const nodeBWidth =
                  nodeB.data?.width || (nodeB.style?.width as number) || NODE_WIDTH;
