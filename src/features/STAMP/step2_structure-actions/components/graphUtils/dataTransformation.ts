...
export const transformAnalysisData = (
  ...
  const getNodeWidth = (controller: Controller): number => {
    const isComplexTeam =
      controller.ctrlType === ControllerType.Team &&
  ...
  controllers.forEach(controller => {
    ...
    if (isComplexTeam) {
      ...
      const containerHeight =
        TEAM_NODE_HEADER_HEIGHT +
        members.length * (MEMBER_NODE_HEIGHT + MEMBER_NODE_SPACING) +
        TEAM_NODE_PADDING;
  ...
  controlPaths.forEach(path => {
    ...
    const isGrandchildConnection =
      !children.includes(path.targetId) && directGrandchildren.includes(path.targetId);
