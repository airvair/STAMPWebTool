...
const ScopeBuilder: React.FC<ScopeBuilderProps> = ({ scope, setScope, handleScopeBlur }) => {
  const title = 'Define the Investigation Scope';
  const description = (
    <>
      Let&apos;s start by drawing the map for our investigation. What are the boundaries?
      <br />
      This defines what&apos;s &apos;in-play&apos; and helps focus our efforts on what we can
      actually influence.
    </>
  );
  ...
  return (
    <CastStepLayout title={title} description={description}>
      ...
      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
        Example: This investigation covers the flight crew and air traffic controllers involved in
        the landing incident at XYZ airport on Jan 1, 2024. It excludes third-party maintenance
