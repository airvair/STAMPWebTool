...
import { AutoExpandingTextarea } from '@/components/shared';
    ...
    <CastStepLayout title={title} description={description}>
        ...
        {sortedEvents.length === 0 && (
          <div className="rounded-lg border-2 border-dashed border-slate-300 p-4 text-center dark:border-slate-600">
            <p className="text-sm text-slate-500">
              No events added yet. Start by adding the first event below.
            </p>
          </div>
        )}
        ...
        <ul ref={sortableListRef} className="space-y-2">
          {sortedEvents.map((event, index) => (
            <li
              key={event.id}
              data-id={event.id}
              className="flex items-start space-x-2 rounded-md border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-900/50"
            >
              {/* Drag handle */}
              <div className="drag-handle mt-1 p-1 text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300">
                <GripVerticalIcon />
              </div>
              <span className="mt-2 w-6 text-right font-mono text-slate-500 dark:text-slate-400">
                {index + 1}.
              </span>
              <Textarea
              ...
              <Button
                ...
                className="mt-1 p-1 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50"
              >
      ...
      <div className="flex items-start space-x-2">
        <AutoExpandingTextarea
          ...
          onKeyPress={e =>
            e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleAddEvent())
          }
          minRows={1}
          maxRows={5}
        />
        <Button onClick={handleAddEvent} leftIcon={<PlaceholderPlusIcon />}>
          Add Event
        </Button>
      </div>
      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
        <strong>Tip:</strong> Avoid the word &apos;fail&apos; unless a mechanical part broke. (e.g.,
        &quot;The pilot did not extend landing gear&quot; instead of &quot;The pilot failed
        to...&quot;).
      </p>
    </CastStepLayout>
  );
