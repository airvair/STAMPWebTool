...
import { InfoPopup } from '@/components/shared';
import { Textarea } from '@/components/shared';
import { Hazard, Loss, AnalysisType } from '@/types/types';
import { hazardInfoContent } from './cast-info';
...
const HazardsBuilder: React.FC<HazardsBuilderProps> = ({
  ...
  return (
    <CastStepLayout title={title} description={description}>
      ...
      <div className="mb-6 rounded-lg border border-slate-200 bg-white p-4 shadow-inner dark:border-slate-700/80 dark:bg-slate-900/50">
        <Textarea
        ...
        <div className="mt-4">
          <label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">
            Which loss(es) can this hazard lead to?
          </label>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {losses.map(loss => (
              <Checkbox
                ...
                checked={linkedLossIds.includes(loss.id)}
        ...
        {hazardError && <p className="mt-2 text-sm text-red-500">{hazardError}</p>}
      </div>

      <div>
        <h4 className="mb-2 text-center text-lg font-semibold text-slate-700 dark:text-slate-200">
          Defined Hazards
        </h4>
        {hazards.length === 0 && (
