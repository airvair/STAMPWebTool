...
const StepItem: React.FC<StepItemProps> = ({
  ...
  const colorClasses = {
    ...
    incomplete:
      'bg-slate-300 dark:bg-slate-600 hover:bg-slate-400 dark:hover:bg-slate-500 text-slate-600 dark:text-slate-300',
  ...
  const renderStepIcon = () => {
    ...
    if (step.status === 'current') return <span className="h-2 w-2 rounded-full bg-white" />;
  ...
  return (
    <li className={containerClasses}>
      ...
      {orientation === 'horizontal' && (
        <span className="mt-2 max-w-16 text-center text-xs leading-tight text-slate-600 dark:text-slate-400">
          {step.label}
        </span>
      )}
...
const ProgressBar: React.FC<ProgressBarProps> = ({ currentStep, totalSteps, orientation }) => {
  ...
  return (
    <div className="mb-6 h-1 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
      <div
