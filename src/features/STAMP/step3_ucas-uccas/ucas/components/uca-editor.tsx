...
import { generateUCACode } from '@/utils/code-generator';
import { generateSmartUcaSuggestions } from '@/utils/smart-uca-suggestions';
import { validateUCA } from '@/utils/uca-validation';
...
const UCAEditor: React.FC<UCAEditorProps> = ({
  ...
  const handleUpdateCondition = (id: string, field: keyof ContextCondition, value: string) => {
    setContextConditions(contextConditions.map(c => (c.id === id ? { ...c, [field]: value } : c)));
  };
  ...
  const handleSave = () => {
    // Build final context
    const finalContext = showStructuredContext ? buildContextFromConditions() : contextText;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] max-w-2xl">
        ...
        <ScrollArea className="mt-6 max-h-[calc(90vh-200px)] flex-1">
            ...
            <Card className="border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900">
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="min-w-[80px] text-sm text-gray-600 dark:text-gray-400">
                    Controller:
                  </span>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-start gap-2">
                  <span className="min-w-[80px] pt-1 text-sm text-gray-600 dark:text-gray-400">
                    Type:
                  </span>
                ...
                <div className="space-y-3">
                  ...
                  {contextConditions.map((condition, index) => (
                    <div key={condition.id} className="flex items-center gap-2">
                      {index > 0 && (
                        <span className="w-10 text-sm font-medium text-gray-500">AND</span>
                      )}
                      <div className={cn('grid flex-1 grid-cols-3 gap-2', index > 0 && 'ml-10')}>
                        <input
                          type="text"
                          placeholder="Variable (e.g., Speed)"
                          value={condition.variable}
                          onChange={e =>
                            handleUpdateCondition(condition.id, 'variable', e.target.value)
                          }
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {suggestions.map((suggestion, idx) => (
                    <div
                      key={idx}
                      className="cursor-pointer rounded-md border p-3 hover:bg-gray-50 dark:hover:bg-gray-900"
        ...
        <div className="flex justify-end gap-2 border-t pt-4">
          ...
          <Button onClick={handleSave}>{uca ? 'Update' : 'Create'} UCA</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
