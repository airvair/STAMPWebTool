...
import { UnsafeControlAction, UCCA, HardwareComponent, FailureMode } from '@/types';
...
export const useAuditedAnalysis = () => {
  ...
  const auditedAddUCA = useCallback((uca: Omit<UnsafeControlAction, 'id' | 'code'>) => {
  ...
  const auditedUpdateUCA = useCallback((id: string, updates: Partial<UnsafeControlAction>) => {
  ...
  const auditedDeleteUCA = useCallback((id: string) => {
  ...
  const auditedUpdateHardwareComponent = useCallback((id: string, updates: Partial<HardwareComponent>) => {
  ...
  const auditedDeleteHardwareComponent = useCallback((id: string) => {
  ...
  const auditedAddFailureMode = useCallback((failureMode: Omit<FailureMode, 'id'>) => {
          ...
          validationResults: [
            {
              ...
              message: createdFailureMode.severityLevel === 'Critical' ? 'Critical failure mode requires additional review' : 'Failure mode severity assessed',
