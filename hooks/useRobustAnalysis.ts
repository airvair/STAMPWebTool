...
import { UnsafeControlAction, UCCA } from '@/types';
import { 
  AnalysisState, 
...
export const useRobustAnalysis = (): [RobustAnalysisState, RobustAnalysisActions] => {
  ...
  // Enhanced UCA operations
  const addUCA = useCallback(async (ucaData: Omit<UnsafeControlAction, 'id' | 'code'>): Promise<string | null> => {
    setOperationInProgress(true);
    try {
      const ucaId = await stateManagerRef.current.addUCA(ucaData, controllers, controlActions, hazards);
      if (ucaId) {
  ...
  const updateUCA = useCallback(async (ucaId: string, updates: Partial<UnsafeControlAction>): Promise<boolean> => {
    setOperationInProgress(true);
    try {
      const success = await stateManagerRef.current.updateUCA(ucaId, updates, controllers, controlActions, hazards);
      if (success) {
  ...
  const deleteUCA = useCallback(async (ucaId: string): Promise<boolean> => {
    setOperationInProgress(true);
    try {
  ...
  const clearError = useCallback((_errorId: string): void => {
  ...
  const forceSave = useCallback(async (): Promise<boolean> => {
    if (!isOnline) {
      showInfo('Cannot save - device is offline. Changes will be saved when connection is restored.');
      return false;
    }
